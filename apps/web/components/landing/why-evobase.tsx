    ...
    <section ref={ref} className="px-6 py-28 sm:py-36">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="text-center mb-20">
          ...
          <h2 className="text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight text-balance text-white/35 leading-[1.1] mt-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-20">
          {VALUE_PROPS.map((prop, i) => (
            <div
              key={prop.title}
              className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-8 transition-all duration-700"
              style={{
            ...
            >
              <div className="flex items-start gap-5">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.04] text-white/60">
                  {prop.icon}
                </div>
                <div>
                  <h3 className="text-lg font-semibold text-white">{prop.title}</h3>
                  <p className="mt-2 text-base text-white/40 leading-relaxed">{prop.description}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
          ...
          <div className="divide-y divide-white/[0.04]">
            {COMPARISONS.map((item, i) => (
              <div
                key={i}
                className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-8 transition-all duration-700"
                style={{
                  opacity: isVisible ? 1 : 0,
              ...
              >
                <div className="flex items-start gap-4">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-red-500/10 text-red-400 mt-0.5">
                    <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </span>
                  <p className="text-base text-white/40">{item.problem}</p>
                </div>
                <div className="flex items-start gap-4">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-400 mt-0.5">
                    <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                  </span>
                  <p className="text-base text-white/60">{item.solution}</p>
                </div>
              </div>
            ))}
          </div>
