    ...
    <section ref={ref} className="px-6 py-16 sm:py-20">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="text-center mb-10">
          ...
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-semibold tracking-tight text-balance text-white/35 leading-[1.15] mt-1">
            evobase bulletproofs the code.
          </h2>
          <p className="mt-4 text-sm sm:text-base text-white/40 max-w-lg mx-auto leading-relaxed">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-10">
          {VALUE_PROPS.map((prop, i) => (
            <div
              key={prop.title}
              className="rounded-lg border border-white/[0.06] bg-white/[0.02] p-5 transition-all duration-700"
              style={{
            ...
            >
              <div className="flex items-start gap-3">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-white/[0.08] bg-white/[0.04] text-white/60">
                  {prop.icon}
                </div>
                <div>
                  <h3 className="text-sm font-semibold text-white">{prop.title}</h3>
                  <p className="mt-1 text-xs text-white/40 leading-relaxed">{prop.description}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="rounded-lg border border-white/[0.06] bg-white/[0.02] overflow-hidden">
          ...
          <div className="divide-y divide-white/[0.04]">
            {COMPARISONS.map((item, i) => (
              <div
                key={i}
                className="grid grid-cols-1 lg:grid-cols-2 gap-3 p-5 transition-all duration-700"
                style={{
                  opacity: isVisible ? 1 : 0,
              ...
              >
                <div className="flex items-start gap-2">
                  <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-red-500/10 text-red-400 mt-0.5">
                    <svg className="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </span>
                  <p className="text-xs text-white/40">{item.problem}</p>
                </div>
                <div className="flex items-start gap-2">
                  <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-400 mt-0.5">
                    <svg className="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                  </span>
                  <p className="text-xs text-white/60">{item.solution}</p>
                </div>
              </div>
            ))}
          </div>
