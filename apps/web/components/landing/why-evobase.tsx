      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight text-balance text-white">
            ...
            <br />
            <span className="text-white/40">evobase bulletproofs the code.</span>
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12">
          {VALUE_PROPS.map((prop, i) => (
            <div
              key={prop.title}
              className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5 transition-all duration-500"
              style={{
            ...
            >
              <div className="flex items-start gap-4">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-white/[0.08] bg-white/[0.04] text-white/60">
                  {prop.icon}
                </div>
                <div>
                  <h3 className="text-sm font-semibold text-white">{prop.title}</h3>
                  <p className="mt-1 text-sm text-white/40 leading-relaxed">{prop.description}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
          ...
          <div className="divide-y divide-white/[0.04]">
            {COMPARISONS.map((item, i) => (
              <div
                key={i}
                className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5 transition-all duration-500"
                style={{
                  opacity: isVisible ? 1 : 0,
