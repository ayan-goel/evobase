...
function FeatureCard({
  ...
  return (
    <div
      ref={ref}
      className="rounded-lg border border-white/[0.06] bg-white/[0.02] overflow-hidden transition-all duration-700"
      style={{
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-2">
        <div className="flex flex-col justify-center p-6">
          <h3 className="text-base sm:text-lg font-semibold tracking-tight text-white text-balance">
            {title}
          </h3>
        ...
        <div className="border-t lg:border-t-0 lg:border-l border-white/[0.06] p-5 flex items-center justify-center min-h-[220px]">
          {visual}
        </div>
      </div>
    </div>
...
function LiveStreamVisual() {
  ...
  return (
    <div ref={ref} className="w-full max-w-xs space-y-1.5">
      {events.map((event, i) => (
        <div
          key={i}
          className={`flex items-center gap-2.5 rounded-md border px-3 py-2 transition-all duration-500 ${
            i < activeEvents
        ...
        >
          ...
          <span className="text-[10px] text-white/60 font-mono truncate">{event.label}</span>
          <span className="ml-auto text-[9px] text-white/20 uppercase tracking-wider shrink-0">
            {event.phase}
          </span>
        </div>
      ))}
      ...
      {[
        ...
        >
          <div className="flex-1 min-w-0">
            <p className={`text-[10px] font-medium ${approach.selected ? "text-blue-300" : "text-white/60"}`}>
              {approach.label}
            </p>
          </div>
          <span
          ...
          {approach.selected && (
            <svg className="h-3 w-3 text-blue-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
          )}
        </div>
    ...
    <div className="w-full max-w-xs">
      <div className="rounded-md border border-white/[0.08] bg-white/[0.03] overflow-hidden">
        <div className="flex items-center gap-2 border-b border-white/[0.06] px-3 py-2">
          <div className="flex h-5 w-5 items-center justify-center rounded-full border border-violet-500/30 bg-violet-500/15">
            <span className="text-[9px]" aria-hidden="true">{"✦"}</span>
          </div>
          <span className="text-[10px] font-medium text-white/60">Agent reasoning</span>
        </div>
        <div className="p-3">
          <pre className="text-[10px] leading-relaxed text-white/40 whitespace-pre-wrap font-mono">
