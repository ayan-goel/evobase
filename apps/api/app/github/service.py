...
# Branch naming convention: evobase/proposal-{short_id}
# Using a prefix makes it easy to identify Evobase branches.
BRANCH_PREFIX = "evobase/proposal-"
RUN_BRANCH_PREFIX = "evobase/run-"
...
async def create_pr_for_proposal(repo: Repository, proposal: Proposal) -> str:
    ...
    if proposal.diff:
        commit_message = f"[Evobase] {proposal.title or proposal.summary or 'Code optimization'}"
    ...
    pr_title = f"[Evobase] {proposal.title or proposal.summary or 'Code optimization'}"
...
async def create_pr_for_run(
    ...
    pr_title = f"[Evobase] {len(proposals)} optimization{'s' if len(proposals) != 1 else ''}"
...
def _build_pr_body(proposal: Proposal) -> str:
    ...
    sections.append("---")
    sections.append("*Generated by [Evobase](https://github.com/ayangoel/evobase) — autonomous code optimization.*")
...
def _build_run_pr_body(run: Run, proposals: list[Proposal]) -> str:
    ...
    count = len(proposals)
    sections = [f"## Evobase Optimizations — Run `{str(run.id)[:8]}`\n"]
    sections.append(
        f"This PR contains {count} optimization"
        f"{'s' if count != 1 else ''} generated by Evobase.\n"
    )
